...
%
%   USAGE
%     results = srp_pipeline();                       % uses cfg.dataDir below
%     results = srp_pipeline('DataDir','/path/data'); % override the folder
...
function cfg = srp_config()
    ...
    % ---------------------------------------------------------------
    % DATA LOCATION
    % Point this to the folder that holds your UTM test files
    % (named like  T50_d122_v500_s3.xlsx).  Override at call time with
    %   srp_pipeline('DataDir', '<other folder>')
    % ---------------------------------------------------------------
    cfg.dataDir    = 'C:\Users\TANVI GATTANI\Downloads\data\new_data_2';
...
function [headers, M] = local_read_spreadsheet(filepath)
    ...
    if isempty(raw) && exist('xlsread', 'file')  % Octave io / older MATLAB
        ...
        try [~, ~, raw] = xlsread(filepath); catch, raw = []; end
    end
...
function v = local_cell2num(x)
    ...
    elseif isstring(x); v = str2double(x); else; v = NaN; end
...
function srp_plot_results(D, A, model, res, cfg)
    ...
    catch err; warning('srp_plot_results:fig1','%s', err.message); end
    ...
    catch err; warning('srp_plot_results:fig2','%s', err.message); end
    ...
    catch err; warning('srp_plot_results:fig3','%s', err.message); end
end
        ...
        for d = daysList
            ...
            for v = cfg.strainRates_mmpmin
                ...
                for s = 1:cfg.nSamples
                    ...
                    if strcmpi(local_field(cfg,'syntheticFormat','xlsx'),'csv'); ext='.csv'; else; ext='.xlsx'; end
